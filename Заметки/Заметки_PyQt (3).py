...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        # Подключите кнопку создания к очистке полей ввода
        self.createButton.clicked.connect(self.clear_fields)

        # Подключите кнопку сохранения к методу save_note
        self.saveButton.clicked.connect(self.save_note)

        # Подключите сигнал клика по элементу QListWidget к методу load_note
        self.savedNotesList.itemClicked.connect(self.load_note)

        # Подключите кнопку "Удалить" к методу delete_note
        self.deleteButton.clicked.connect(self.delete_note)

        # Загрузить сохраненные заметки при запуске
        self.load_notes()

        # Настройка кнопки уведомления
